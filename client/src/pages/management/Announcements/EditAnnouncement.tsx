...
import { updateAnnouncement, getAnnouncement } from '@/api/announcements';
...
export default function EditAnnouncement() {
    ...
    const existing = (location.state as { announcement?: Announcement })?.announcement;
    ...
    useEffect(() => {
        Promise.all([getClasses(), existing ? Promise.resolve(null) : getAnnouncement(Number(id))])
            .then(([classesRes, fetched]) => {
                setClasses(classesRes.data.classes);
                const a = existing ?? fetched?.data.announcement ?? null;
                if (a) {
                    setForm({
            ...
            .catch(() => navigate('/management/announcements'))
    ...
    if (loading) {
        return <p className="p-6 text-sm text-muted">Loading announcement...</p>;
    }

    return (
        <div className="max-w-xl">
            <h1 className="text-2xl font-semibold text-ink2">Edit Announcement</h1>
            <p className="mt-1 text-sm text-muted">Update announcement details</p>
